...
def _parse_action_from_filename(filename: str) -> str:
    name_parts = os.path.basename(filename).replace('.png', '').split('_')
    if len(name_parts) > 3:
...
def process_episode_scalevln(ep, img_root, act_map):
    ...
    for i in range(num_images):
        ...
        elif i < len(ep['actions']):
            prev_action = act_map[ep['actions'][i]]
        else:
            prev_action = START_ACTION
...
def process_episode_vlnce(ep, img_root):
    ...
    for i in range(len(img_files)):
        ...
        his_img_tags = "<image>" * (len(sampled_imgs) - 1)
        action = _parse_action_from_filename(img_files[i])
        prev_action = START_ACTION if i == 0 else _parse_action_from_filename(img_files[i - 1])
...
def main():
    ...
    parser.add_argument(
        '--use_R2R_data',
        action='store_true',
        help="Include R2R dataset in the processing.")
    ...
    new_path = "/ssd/dingmuhe/Embodied-task/JanusVLN/"
    # --- R2R Dataset ---
    img_root_r2r = os.path.join(new_path, "data/trajectory_data/R2R/train")
    json_path_r2r = os.path.join(new_path, "data/datasets/r2r/train/train.json.gz")
    print('img_root_r2r:', img_root_r2r)
    # --- RxR Dataset ---
    img_root_rxr = os.path.join(new_path,"data/trajectory_data/RxR/train")
    json_path_rxr = os.path.join(new_path,"data/datasets/rxr/train/train_guide.json.gz")
    print('img_root_rxr:', img_root_rxr)
    ...
    if args.use_extra_data:
        ...
        if args.use_R2R_data:
            output_path = "train_r2r_action.json"
        else:
