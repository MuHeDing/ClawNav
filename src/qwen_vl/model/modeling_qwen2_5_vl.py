...
def debug_print_var(var_name, var_value, indent=0):
    """
    Debug utility to print variable information in modeling_qwen2_5_vl.py
    ...
    separator = "=" * 60
    print(f"\n{prefix}{separator}")
    print(f"{prefix}[DEBUG-MODEL] Variable: {var_name}")
...
class Qwen2_5_VisionTransformerPretrainedModel(Qwen2_5_VLPreTrainedModel):
    ...
    def get_window_index(self, grid_thw):
        ...
        for grid_t, grid_h, grid_w in grid_thw:
            ...
            # # 累积序列长度（用于 Flash Attention 的变长序列）
...
class Qwen2_5_VLForConditionalGenerationForJanusVLN(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        # 从配置中读取 KV 缓存参数，如果配置中没有则使用默认值
        kv_start_size = getattr(config, "kv_start_size", 8)
        kv_recent_size = getattr(config, "kv_recent_size", 24)

        self.kv_cache_vggt = StartRecentKVCache(
                start_size=kv_start_size,
                recent_size=kv_recent_size,
                k_seq_dim=2,
                v_seq_dim=2
            )
    ...
    def get_rope_index(
        ...
        if input_ids is not None and (image_grid_thw is not None or video_grid_thw is not None):
            ...
            # 创建3D位置ID张量，形状为 [3, batch_size, seq_len]，分别对应时间、高度、宽度三个维度
            ...
            for i, input_ids in enumerate(total_input_ids):
                ...
                for _ in range(image_nums + video_nums):
                    ...
                    # 将时间、高度、宽度三个维度堆叠，加上偏移量，更新起始位置。
                    st = ed + llm_grid_t * llm_grid_h * llm_grid_w
                    
                # 如果最后还有文本token，为它们分配位置ID。
    ...
    def forward(
        ...
        **kwargs,
        ...
        if inputs_embeds is None:
            ...
            # DEBUG: Print initial inputs_embeds (text only)
            #debug_print_var("inputs_embeds (initial, text only)", inputs_embeds) # torch.Size([1, 526, 3584])

            if pixel_values is not None:
                batch_size = inputs_embeds.shape[0]   # 测试还是不支持batch处理 KV cache会冲突，训练的应该可以
                image_embeds_3d = []
                teacher_outputs = []
                for i in range(batch_size):
                    if images_vggt[i].shape[0] > 0:
                        # DEBUG: Print images_vggt[i]
                        #debug_print_var(f"images_v ggt[{i}]", images_vggt[i], indent=1) # Shape: torch.Size([1, 3, 490, 644])
            ...
            #  - 如果 inputs_embeds 的形状是 [1, 526, 3584]（batch=1, seq_len=526, hidden_dim=3584）
            #    则 attention_mask 的形状是 [1, 526]
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        #print('Begin prepare_inputs_for_generation')
        ...
        #print('End prepare_inputs_for_generation')
