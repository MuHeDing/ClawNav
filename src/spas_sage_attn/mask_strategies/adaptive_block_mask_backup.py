...
class AdaptiveBlockMasker:
    ...
    def _finalize_tail_block(self, state: LayerState) -> None:
        ...
        m2_block = state.tail_sq_norm - block_width * mu_sq_sum
        m2_block = torch.clamp(m2_block, min=0.0)
        ...
        state.m2_sum = torch.cat([state.m2_sum, m2_block], dim=1)
    ...
    def _apply_chunks(
        ...
        if new_len >= block_width:
            ...
            if num_full > 0:
                ...
                sq_norm = k_full.square().sum(dim=(2, 3))

                mu_new = k_sum / float(block_width)
                vbar_new = v_sum / float(block_width)
                mu_sq = mu_new.square().sum(dim=-1)
                m2_new = torch.clamp(sq_norm - float(block_width) * mu_sq, min=0.0)
                gamma_new = (m2_new / float(max(block_width - 1, 1))) / float(self.head_dim)
                ...
                state.m2_sum = torch.cat([state.m2_sum, m2_new], dim=1)
    ...
    def compute_mask(
        ...
        if multi_query:
            ...
            q_sq = (q_vec * q_vec).sum(dim=-1)
            q_norm = torch.sqrt(q_sq.clamp_min(self.epsilon))
            mean_norm = q_norm.mean(dim=-1, keepdim=True).clamp_min(self.epsilon)
            clip_ratio = self.q_clip_ratio
            upper = mean_norm * clip_ratio
            lower = mean_norm / clip_ratio

            shrink = torch.ones_like(q_norm)
            shrink = torch.where(q_norm > upper, upper / q_norm, shrink)
            shrink = torch.where(q_norm < lower, lower / q_norm, shrink)
            q_vec = q_vec * shrink.unsqueeze(-1)

            scale = self.inv_sqrt_dim
            q_norm2 = (q_vec * q_vec).sum(dim=-1) * (scale * scale)
            logits = torch.einsum('hqd,hkd->hqk', q_vec, mu_nr) * scale
        ...
        if prefill:
            q_sq = (q_vec * q_vec).sum(dim=-1)
            q_norm = torch.sqrt(q_sq.clamp_min(self.epsilon))
            mean_norm = q_norm.mean().clamp_min(self.epsilon)
            clip_ratio = self.q_clip_ratio
            upper = mean_norm * clip_ratio
            lower = mean_norm / clip_ratio
            ...
            logits = torch.matmul(mu_nr, q_vec.unsqueeze(-1)).squeeze(-1) * scale
            logits = logits / tau
            tau_sq = tau * tau
