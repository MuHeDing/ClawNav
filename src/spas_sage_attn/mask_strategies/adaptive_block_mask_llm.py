...
class LayerState:
    ...
    mu: torch.Tensor  # (H, B, D)
    vbar: torch.Tensor  # (H, B, D)
    gamma: torch.Tensor  # (H, B)
...
class AdaptiveBlockMasker:
    ...
    def _init_layer_state(self) -> LayerState:
        ...
        return LayerState(
            ...
            tail_sq_norm=torch.zeros((self.num_heads,), device=self.device, dtype=torch.float32),
    ...
    def _accumulate_tail(
        ...
        state.tail_sq_norm.add_(chunk_k.square().sum(dim=(1, 2)))
    ...
    def _finalize_tail_block(self, state: LayerState) -> None:
        ...
        denom = max(self.kv_block_size - 1, 1)

        mu_block = state.tail_k_sum / block_width
        vbar_block = state.tail_v_sum / block_width

        mu_sq_sum = mu_block.square().sum(dim=-1)
        m2_block = torch.clamp(state.tail_sq_norm - block_width * mu_sq_sum, min=0.0)
        gamma_block = (m2_block / float(denom)) / float(self.head_dim)

        mu_block = mu_block.unsqueeze(1)
        vbar_block = vbar_block.unsqueeze(1)
        m2_block = m2_block.unsqueeze(1)
        gamma_block = gamma_block.unsqueeze(1)
    ...
    def _apply_chunks(
        ...
        if new_len >= block_width:
            ...
            if num_full > 0:
                ...
                mu_new = k_full.mean(dim=2)
                vbar_new = v_full.mean(dim=2)

                var_full = k_full.var(dim=2, unbiased=True)
                gamma_new = var_full.mean(dim=-1)
    ...
    def compute_mask(
        ...
        if multi_query:
            ...
            mu_nr = state.mu[:, :nr_end, :].to(torch.float32)
            gamma_nr = state.gamma[:, :nr_end].to(torch.float32)
            tau = state.tau.clamp_min(self.epsilon).to(torch.float32)

            mu_scale = mu_nr.norm(dim=-1).mean(dim=1, keepdim=True).clamp_min(self.epsilon)
            mu_normed = mu_nr / mu_scale.unsqueeze(-1)
            gamma_nr = gamma_nr / (mu_scale * mu_scale)

            q_scale = q_vec.norm(dim=-1).mean(dim=1, keepdim=True).clamp_min(self.epsilon)
            ...
            if debug_masks:
                ...
                mu_norm_vals = mu_normed.norm(dim=-1)
                mu_norm_min = mu_norm_vals.amin(dim=-1)
                mu_norm_max = mu_norm_vals.amax(dim=-1)
                mu_norm_avg = mu_norm_vals.mean(dim=-1)
                print(
                ...
                print(
                    f"  mu_norm[min={mu_norm_min.mean().item():.3f}, max={mu_norm_max.mean().item():.3f}, avg={mu_norm_avg.mean().item():.3f}]"
                )
        ...
        mu_scale = mu_nr.norm(dim=-1).mean(dim=1, keepdim=True).clamp_min(self.epsilon)
        mu_normed = mu_nr / mu_scale.unsqueeze(-1)
        gamma_scaled = gamma_nr / (mu_scale * mu_scale)
        ...
        if prefill:
            ...
            g_nr.scatter_(
                dim=-1, index=sorted_idx, src=mask_sorted
            )
            ...
            if debug_masks:
                ...
                logits_min = logits.amin(dim=-1)
                logits_max = logits.amax(dim=-1)
                gamma_min = gamma_scaled.amin(dim=-1)
                gamma_max = gamma_scaled.amax(dim=-1)
                qnorm_avg = q_norm2
                mu_norm_vals = mu_normed.norm(dim=-1)
                mu_norm_min = mu_norm_vals.amin(dim=-1)
                mu_norm_max = mu_norm_vals.amax(dim=-1)
            ...
            phi_raw = torch.sum((vbar_nr - y_hat.unsqueeze(1)) ** 2, dim=-1)
            ...
            if debug_masks:
                kept_counts = g_nr_bool.sum(dim=-1).float()
                kept_mass = (m_tilde * g_nr_bool.float()).sum(dim=-1)
                phi_mean = phi.mean(dim=-1)
                phi_std = phi.std(dim=-1, unbiased=False)
                print(
                    f"[AdaptiveMask][layer={layer}] decode keep avg={kept_counts.mean().item():.2f}"
                    f" min={kept_counts.min().item():.0f} max={kept_counts.max().item():.0f}"
                    f" mass_avg={kept_mass.mean().item():.3f} drop_avg={dropped_mass.mean().item():.3f}"
                )
                print(
                    f"  phi_mean_avg={phi_mean.mean().item():.3f} phi_std_avg={phi_std.mean().item():.3f}"
                )
