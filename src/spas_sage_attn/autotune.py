...
class SparseAttentionMeansim(nn.Module):
    ...
    def forward(
        ...
        if os.environ.get("TUNE_MODE", "") != "" or tune_mode:
            ...
            kernel = self.kernel_selection()
            record_flag = return_sparsity or self.log_sparsity
            o = kernel(
                q,
                k,
                v,
                ...
                return_sparsity=record_flag,
                attention_sink= True,  # Only keep True when inference !!!!
            )
        
        if return_sparsity or self.log_sparsity:
            o, total_sparsity = o
            if self.log_sparsity:
                self.sparsity_records.append(float(total_sparsity))
            if return_sparsity:
