...
args=(
  ...
  --qwen_thinking_interval_steps "${OPENCLAW_QWEN_THINKING_INTERVAL_STEPS}"
  --qwen_output_schema "${OPENCLAW_QWEN_OUTPUT_SCHEMA}"
  --qwen_transport_mode "${OPENCLAW_QWEN_TRANSPORT_MODE}"
  --segmentation_timeout "${OPENCLAW_SEGMENTATION_TIMEOUT}"
...
if [[ -n "${OPENCLAW_STAGE_PLAN_MANIFEST_PATH}" ]]; then
  args+=(--stage_plan_manifest_path "${OPENCLAW_STAGE_PLAN_MANIFEST_PATH}")
fi
