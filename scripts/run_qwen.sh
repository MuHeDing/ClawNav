...
QWEN_STAGE_PLAN_CAPTURE=${QWEN_STAGE_PLAN_CAPTURE:-0}
QWEN_ABLATION_PROFILE=${QWEN_ABLATION_PROFILE:-}
if [[ -n "${QWEN_ABLATION_PROFILE}" ]]; then
  case "${QWEN_ABLATION_PROFILE}" in
    fixed_off)
      OPENCLAW_DYNAMIC_VISUAL_CONTEXT_ENABLED=0
      OPENCLAW_QWEN_THINKING_MODE=off
      ;;
    dynamic_off)
    ...
    fixed_on)
      OPENCLAW_DYNAMIC_VISUAL_CONTEXT_ENABLED=0
      OPENCLAW_QWEN_THINKING_MODE=on
      ;;
    dynamic_on)
      OPENCLAW_DYNAMIC_VISUAL_CONTEXT_ENABLED=1
      OPENCLAW_QWEN_THINKING_MODE=on
    ...
    staged_memory_on)
      ...
      OPENCLAW_STAGED_MEMORY_TREATMENT=on
      OPENCLAW_DYNAMIC_VISUAL_CONTEXT_ENABLED=1
      OPENCLAW_QWEN_OUTPUT_SCHEMA=route_v3_staged
      ;;
    staged_memory_off)
      OPENCLAW_STAGED_VISUAL_MEMORY_ENABLED=1
      OPENCLAW_STAGED_MEMORY_TREATMENT=off_ablation
      OPENCLAW_DYNAMIC_VISUAL_CONTEXT_ENABLED=1
      OPENCLAW_QWEN_OUTPUT_SCHEMA=route_v3_staged
      ;;
    *)
      echo "Unsupported QWEN_ABLATION_PROFILE: ${QWEN_ABLATION_PROFILE}" >&2
      exit 2
      ;;
  esac
  OPENCLAW_MODEL=qwen/qwen3.5-flash-2026-02-23
  if [[ "${QWEN_ABLATION_PROFILE}" != staged_memory_* ]]; then
    OPENCLAW_QWEN_OUTPUT_SCHEMA=route_v2
  fi
  OPENCLAW_QWEN_TRANSPORT_MODE=sync
  OPENCLAW_QWEN_THINKING_BUDGET=1024
  OPENCLAW_MODEL_FAST_MODE=off
  OPENCLAW_MODEL_IMAGE_INTERVAL_STEPS=1
  OPENCLAW_MAP_ASSIST_MODE=floorplan_map_assisted
  OPENCLAW_MAP_FRAME_INTERVAL_STEPS=5
  OPENCLAW_MOTION_FEEDBACK_ENABLED=1
  OPENCLAW_FORWARD_STALL_ODOMETRY_ENABLED=1
  OPENCLAW_MAP_COLLISION_OVERLAY_ENABLED=1
fi
if [[ "${OPENCLAW_STAGED_VISUAL_MEMORY_ENABLED}" == "1" ]]; then
  if [[ "${OPENCLAW_QWEN_OUTPUT_SCHEMA}" != "route_v3_staged" ]]; then
    echo "Staged visual memory requires OPENCLAW_QWEN_OUTPUT_SCHEMA=route_v3_staged" >&2
    exit 2
  fi
  if [[ "${OPENCLAW_DYNAMIC_VISUAL_CONTEXT_ENABLED}" != "1" ]]; then
    echo "Staged visual memory requires OPENCLAW_DYNAMIC_VISUAL_CONTEXT_ENABLED=1" >&2
    exit 2
  fi
  if [[ "${QWEN_ABLATION_PROFILE}" == staged_memory_* && "${QWEN_STAGE_PLAN_CAPTURE}" != "1" && ( -z "${OPENCLAW_STAGE_PLAN_MANIFEST_PATH}" || ! -f "${OPENCLAW_STAGE_PLAN_MANIFEST_PATH}" ) ]]; then
    echo "Staged visual memory requires a frozen OPENCLAW_STAGE_PLAN_MANIFEST_PATH" >&2
    exit 2
  fi
fi
if [[ "${QWEN_STAGE_PLAN_CAPTURE}" == "1" ]]; then
  if [[ -z "${DATA_PATH:-}" || -z "${OPENCLAW_STAGE_PLAN_MANIFEST_PATH}" ]]; then
    echo "QWEN_STAGE_PLAN_CAPTURE=1 requires DATA_PATH and OPENCLAW_STAGE_PLAN_MANIFEST_PATH" >&2
    exit 2
  fi
  GATEWAY_STAGE_PLAN_MANIFEST_PATH=""
else
  GATEWAY_STAGE_PLAN_MANIFEST_PATH="${OPENCLAW_STAGE_PLAN_MANIFEST_PATH}"
fi
...
OPENCLAW_QWEN_OUTPUT_SCHEMA="${OPENCLAW_QWEN_OUTPUT_SCHEMA:-legacy}" \
OPENCLAW_QWEN_TRANSPORT_MODE="${OPENCLAW_QWEN_TRANSPORT_MODE:-sync}" \
OPENCLAW_SEGMENTATION_TIMEOUT="${OPENCLAW_SEGMENTATION_TIMEOUT:-120}" \
OPENCLAW_STAGE_PLAN_MANIFEST_PATH="${GATEWAY_STAGE_PLAN_MANIFEST_PATH}" \
...
if [[ "${QWEN_STAGE_PLAN_CAPTURE}" == "1" ]]; then
  PYTHONPATH=src:. python scripts/generate_qwen_stage_plan_manifest.py \
    --data-path "${DATA_PATH}" \
    --episode-keys "${EPISODE_KEYS}" \
    --gateway-url "${OPENCLAW_GATEWAY_URL}" \
    --timeout "${OPENCLAW_SEGMENTATION_TIMEOUT:-120}" \
    --output "${OPENCLAW_STAGE_PLAN_MANIFEST_PATH}"
  echo "Frozen stage plan written to ${OPENCLAW_STAGE_PLAN_MANIFEST_PATH}"
  exit 0
fi
EVAL_VARS=(
  ...
  OPENCLAW_DYNAMIC_VISUAL_CONTEXT_ENABLED="${OPENCLAW_DYNAMIC_VISUAL_CONTEXT_ENABLED:-0}"
  OPENCLAW_STAGED_VISUAL_MEMORY_ENABLED="${OPENCLAW_STAGED_VISUAL_MEMORY_ENABLED}"
  OPENCLAW_STAGED_MEMORY_TREATMENT="${OPENCLAW_STAGED_MEMORY_TREATMENT}"
  OPENCLAW_STAGED_SHADOW_MANIFEST_PATH="${OPENCLAW_STAGED_SHADOW_MANIFEST_PATH}"
  ...
  OPENCLAW_QWEN_OUTPUT_SCHEMA="${OPENCLAW_QWEN_OUTPUT_SCHEMA}"
