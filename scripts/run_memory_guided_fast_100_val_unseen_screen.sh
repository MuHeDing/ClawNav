...
MEMORY_POLICY_MODE=${MEMORY_POLICY_MODE:-raw}
FILTER_STOP_SEMANTICS=${FILTER_STOP_SEMANTICS:-0}
STOP_VERIFICATION_MODE=${STOP_VERIFICATION_MODE:-off}
FAST_USE_MEMORY_CONTEXT=${FAST_USE_MEMORY_CONTEXT:-1}
OPENCLAW_POLICY_MEMORY_CONTEXT_ENABLED=${OPENCLAW_POLICY_MEMORY_CONTEXT_ENABLED:-}
EPISODE_KEYS_PATH=${EPISODE_KEYS_PATH:-}
...
export MEMORY_POLICY_MODE FILTER_STOP_SEMANTICS STOP_VERIFICATION_MODE FAST_USE_MEMORY_CONTEXT
export OPENCLAW_POLICY_MEMORY_CONTEXT_ENABLED EPISODE_KEYS_PATH
...
runner_args=(
  ...
  --memory-policy-mode "${MEMORY_POLICY_MODE}"
  --filter-stop-semantics "${FILTER_STOP_SEMANTICS}"
  --stop-verification-mode "${STOP_VERIFICATION_MODE}"
  --fast-use-memory-context "${FAST_USE_MEMORY_CONTEXT}"
...
if [[ -n "${OPENCLAW_POLICY_MEMORY_CONTEXT_ENABLED}" ]]; then
  runner_args+=(--openclaw-policy-memory-context-enabled "${OPENCLAW_POLICY_MEMORY_CONTEXT_ENABLED}")
fi
if [[ -n "${EPISODE_KEYS_PATH}" ]]; then
  runner_args+=(--episode-keys-path "${EPISODE_KEYS_PATH}")
fi
